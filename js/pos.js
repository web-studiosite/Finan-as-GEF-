...
export async function renderPosView(container) {
    ...
    <div class="flex flex-col xl:flex-row gap-5 h-auto xl:h-[calc(100vh-6.5rem)] min-h-[580px]">
      ...
      <div class="flex-1 flex flex-col min-w-0 bg-slate-900 border border-slate-800 rounded-2xl p-4 overflow-hidden shadow-lg">
        ...
        <div class="space-y-3 pb-3 border-b border-slate-800">
          ...
          <div id="posCategoriesBar" class="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none text-xs">
            <button class="cat-btn active px-3 py-1 rounded-lg bg-orange-600 text-white font-semibold whitespace-nowrap cursor-pointer" data-cat="TODAS">
              Todas
            </button>
          </div>
        </div>
      ...
      <div class="w-full xl:w-96 shrink-0 flex flex-col bg-slate-900 border border-slate-800 rounded-2xl p-4 overflow-hidden shadow-xl">
        ...
        <div class="flex items-center justify-between pb-3 border-b border-slate-800">
          ...
          <button id="btnClearCartBtn" class="text-[11px] text-slate-400 hover:text-red-400 flex items-center gap-1 transition cursor-pointer">
        ...
        <div class="pt-3 border-t border-slate-800 space-y-2.5">
          ...
          <div>
            ...
            <div id="posPaymentMethods" class="grid grid-cols-3 gap-1 text-[10px] font-bold">
              <button class="pay-btn active py-1.5 px-1 rounded-lg bg-orange-600 text-white border border-orange-500 text-center cursor-pointer" data-method="DINHEIRO">
                Dinheiro
              </button>
              <button class="pay-btn py-1.5 px-1 rounded-lg bg-slate-800 text-slate-300 border border-slate-700 hover:bg-slate-700 text-center cursor-pointer" data-method="M-PESA">
                M-Pesa
              </button>
              <button class="pay-btn py-1.5 px-1 rounded-lg bg-slate-800 text-slate-300 border border-slate-700 hover:bg-slate-700 text-center cursor-pointer" data-method="E-MOLA">
                e-Mola
              </button>
              <button class="pay-btn py-1.5 px-1 rounded-lg bg-slate-800 text-slate-300 border border-slate-700 hover:bg-slate-700 text-center cursor-pointer" data-method="CARTAO">
                Cartão POS
              </button>
              <button class="pay-btn py-1.5 px-1 rounded-lg bg-slate-800 text-slate-300 border border-slate-700 hover:bg-slate-700 text-center cursor-pointer" data-method="TRANSFERENCIA">
                Transferência
              </button>
              <button class="pay-btn py-1.5 px-1 rounded-lg bg-slate-800 text-amber-400 border border-slate-700 hover:bg-slate-700 text-center cursor-pointer" data-method="CREDITO_FIADO">
                Fiado / Obra
              </button>
            </div>
          </div>
          ...
          <button
            id="btnFinalizeSale"
            class="w-full py-3 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-500 hover:to-amber-500 disabled:opacity-50 text-white font-bold text-xs shadow-lg shadow-orange-950/40 flex items-center justify-center gap-2 transition active:scale-98 cursor-pointer"
          >
      ...
      return `
        <div class="py-2 space-y-1">
          ...
          <div class="flex items-center justify-between mt-1">
            ...
            <button class="btn-remove-item text-slate-500 hover:text-red-400 p-1 transition cursor-pointer" data-index="${idx}">
              <i data-lucide="trash-2" class="w-3.5 h-3.5"></i>
            </button>
          </div>
        </div>
  ...
  const loadData = async () => {
    ...
    categoriesBar.innerHTML = cats.map(c => `
      <button class="cat-btn px-3 py-1 rounded-lg ${c === selectedCategory ? 'bg-orange-600 text-white font-semibold' : 'bg-slate-800 text-slate-400 hover:text-white'} whitespace-nowrap cursor-pointer" data-cat="${c}">
        ${c}
      </button>
    `).join('');
    ...
    categoriesBar.querySelectorAll('.cat-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        categoriesBar.querySelectorAll('.cat-btn').forEach(b => {
          b.className = `cat-btn px-3 py-1 rounded-lg ${b.dataset.cat === selectedCategory ? 'bg-orange-600 text-white font-semibold' : 'bg-slate-800 text-slate-400 hover:text-white'} whitespace-nowrap cursor-pointer`;
        });
        renderCatalog();
      });
    });
  ...
  paymentButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      paymentButtons.forEach(b => {
        b.className = 'pay-btn py-1.5 px-1 rounded-lg bg-slate-800 text-slate-300 border border-slate-700 hover:bg-slate-700 text-center cursor-pointer';
      });
      btn.className = 'pay-btn active py-1.5 px-1 rounded-lg bg-orange-600 text-white border border-orange-500 text-center cursor-pointer';
