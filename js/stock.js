...
export async function renderStockView(container) {
    ...
    <div class="space-y-5">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl bg-slate-900 border border-slate-800">
        ...
        <div class="flex items-center gap-2">
          <button id="btnRefreshStock" class="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold transition border border-slate-700 cursor-pointer">
            <i data-lucide="refresh-cw" class="w-3.5 h-3.5"></i>
            <span>Atualizar</span>
          </button>
          <button id="btnOpenTransferGlobal" class="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-orange-600 hover:bg-orange-500 text-white text-xs font-bold shadow-md shadow-orange-950/40 transition active:scale-95 cursor-pointer">
      ...
      <div class="flex flex-col sm:flex-row gap-3">
        ...
        <div class="flex rounded-xl bg-slate-900 border border-slate-800 p-1 text-xs font-semibold">
          <button id="filterAllStock" class="stock-tab-btn active px-3 py-1.5 rounded-lg bg-orange-600 text-white transition cursor-pointer">
            Todos os Materiais
          </button>
          <button id="filterCriticalStock" class="stock-tab-btn px-3 py-1.5 rounded-lg text-slate-400 hover:text-white transition cursor-pointer">
            Abaixo do Mínimo
          </button>
        </div>
      </div>
      ...
      return `
        <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800 hover:border-slate-700 transition space-y-3 shadow-xs">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            <div class="flex items-center gap-2">
              <button class="btn-transfer-item flex items-center gap-1 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold border border-slate-700 transition cursor-pointer" data-id="${p.id}">
  ...
  filterAllBtn.addEventListener('click', () => {
    filterMode = 'ALL';
    filterAllBtn.className = 'stock-tab-btn active px-3 py-1.5 rounded-lg bg-orange-600 text-white transition cursor-pointer';
    filterCriticalBtn.className = 'stock-tab-btn px-3 py-1.5 rounded-lg text-slate-400 hover:text-white transition cursor-pointer';
    renderCards();
  });

  filterCriticalBtn.addEventListener('click', () => {
    filterMode = 'CRITICAL';
    filterCriticalBtn.className = 'stock-tab-btn active px-3 py-1.5 rounded-lg bg-orange-600 text-white transition cursor-pointer';
    filterAllBtn.className = 'stock-tab-btn px-3 py-1.5 rounded-lg text-slate-400 hover:text-white transition cursor-pointer';
    renderCards();
  });
    ...
    <div class="relative w-full max-w-lg rounded-2xl bg-slate-900 border border-slate-700 shadow-2xl overflow-hidden flex flex-col text-xs text-slate-200">
      <div class="flex items-center justify-between px-6 py-4 bg-slate-800/90 border-b border-slate-700">
        <div class="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p class="text-xs text-slate-400">Movimentação física com atualização no PostgreSQL</p>
          </div>
        </div>
        <button id="closeTransferModalBtn" class="p-1 rounded-lg text-slate-400 hover:text-white cursor-pointer">
      ...
      <form id="transferForm" class="p-6 space-y-4">
        ...
        <div class="flex gap-3 pt-2">
          <button type="button" id="cancelTransferBtn" class="flex-1 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold text-xs cursor-pointer">
            Cancelar
          </button>
          <button type="submit" id="submitTransferBtn" class="flex-2 py-3 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-500 hover:to-amber-500 text-white font-bold text-xs shadow-lg shadow-orange-950/50 cursor-pointer">
