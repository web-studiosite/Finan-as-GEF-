...
export async function openReceiptModal(saleData) {
    ...
    <div class="relative w-full max-w-sm rounded-2xl bg-slate-900 border border-slate-700 shadow-2xl overflow-hidden flex flex-col my-auto">
      ...
      <div class="flex items-center justify-between px-4 py-3 bg-slate-800 border-b border-slate-700">
        ...
        <div class="flex items-center gap-1.5">
          <button id="btnPrintReceiptNow" class="px-3 py-1 rounded-lg bg-orange-600 hover:bg-orange-500 text-white font-bold text-xs flex items-center gap-1 shadow-xs transition cursor-pointer">
            <i data-lucide="printer" class="w-3.5 h-3.5"></i>
            <span>Imprimir</span>
          </button>
          <button id="btnCloseReceiptModal" class="p-1 rounded-lg text-slate-400 hover:text-white transition cursor-pointer">
  ...
  modalContainer.querySelector('#btnPrintReceiptNow').addEventListener('click', () => {
    // Janela limpa para impressão térmica
...
const RECEIPT_CONFIG_STORAGE_KEY = 'gef_thermal_receipt_config';

export function getReceiptConfig() {
  try {
    const raw = localStorage.getItem(RECEIPT_CONFIG_STORAGE_KEY);
    if (raw) return JSON.parse(raw);
  } catch (e) {}
  return {
    paperWidth: '80mm',
    storeName: 'GEF MATERIAIS DE CONSTRUÇÃO',
    nuit: '400123987',
    phone: '+258 84 000 0000',
    address: 'Av. de Moçambique, Maputo',
    footerMessage: 'Obrigado pela preferência! Guarde este recibo.'
  };
}

export function saveReceiptConfig(config) {
  try {
    localStorage.setItem(RECEIPT_CONFIG_STORAGE_KEY, JSON.stringify(config));
  } catch (e) {}
}
